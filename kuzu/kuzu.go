...
func DatabaseDestroy(database []Database) {
	cdatabase, cdatabaseAllocMap := unpackArgSDatabase(database)
	C.kuzu_database_destroy(cdatabase)
	packSDatabase(database, cdatabase)
...
func ConnectionDestroy(connection []Connection) {
	cconnection, cconnectionAllocMap := unpackArgSConnection(connection)
	C.kuzu_connection_destroy(cconnection)
	packSConnection(connection, cconnection)
...
func ConnectionExecute(connection []Connection, preparedStatement []PreparedStatement) *QueryResult {
	cconnection, cconnectionAllocMap := unpackArgSConnection(connection)
	cpreparedStatement, cpreparedStatementAllocMap := unpackArgSPreparedStatement(preparedStatement)
	__ret := C.kuzu_connection_execute(cconnection, cpreparedStatement)
	packSPreparedStatement(preparedStatement, cpreparedStatement)
...
func QueryResultDestroy(queryResult []QueryResult) {
	cqueryResult, cqueryResultAllocMap := unpackArgSQueryResult(queryResult)
	C.kuzu_query_result_destroy(cqueryResult)
	packSQueryResult(queryResult, cqueryResult)
...
func FlatTupleDestroy(flatTuple []FlatTuple) {
	cflatTuple, cflatTupleAllocMap := unpackArgSFlatTuple(flatTuple)
	C.kuzu_flat_tuple_destroy(cflatTuple)
	packSFlatTuple(flatTuple, cflatTuple)
...
func DataTypeCreate(id DataTypeId, childType []LogicalType, numElementsInArray uint64) *LogicalType {
	...
	cchildType, cchildTypeAllocMap := unpackArgSLogicalType(childType)
	...
	packSLogicalType(childType, cchildType)
...
func DataTypeClone(dataType []LogicalType) *LogicalType {
	cdataType, cdataTypeAllocMap := unpackArgSLogicalType(dataType)
	__ret := C.kuzu_data_type_clone(cdataType)
	packSLogicalType(dataType, cdataType)
...
func DataTypeEquals(dataType1 []LogicalType, dataType2 []LogicalType) bool {
	cdataType1, cdataType1AllocMap := unpackArgSLogicalType(dataType1)
	cdataType2, cdataType2AllocMap := unpackArgSLogicalType(dataType2)
	__ret := C.kuzu_data_type_equals(cdataType1, cdataType2)
	packSLogicalType(dataType2, cdataType2)
	runtime.KeepAlive(cdataType2AllocMap)
	packSLogicalType(dataType1, cdataType1)
...
func ValueCopy(value []Value, other []Value) {
	cvalue, cvalueAllocMap := unpackArgSValue(value)
	cother, cotherAllocMap := unpackArgSValue(other)
	C.kuzu_value_copy(cvalue, cother)
	packSValue(other, cother)
...
func NodeValGetIdVal(nodeVal []Value) *Value {
	cnodeVal, cnodeValAllocMap := unpackArgSValue(nodeVal)
	__ret := C.kuzu_node_val_get_id_val(cnodeVal)
	packSValue(nodeVal, cnodeVal)
...
func RelValGetSrcIdVal(relVal []Value) *Value {
	crelVal, crelValAllocMap := unpackArgSValue(relVal)
	__ret := C.kuzu_rel_val_get_src_id_val(crelVal)
	packSValue(relVal, crelVal)
...
func RdfVariantGetType(rdfVariant []Value) DataTypeId {
	crdfVariant, crdfVariantAllocMap := unpackArgSValue(rdfVariant)
	__ret := C.kuzu_rdf_variant_get_type(crdfVariant)
	packSValue(rdfVariant, crdfVariant)
...
func QuerySummaryDestroy(querySummary []QuerySummary) {
	cquerySummary, cquerySummaryAllocMap := unpackArgSQuerySummary(querySummary)
	C.kuzu_query_summary_destroy(cquerySummary)
	packSQuerySummary(querySummary, cquerySummary)
